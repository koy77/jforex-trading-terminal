...
### Added
- **Pending Order Pattern (P Key)** - новый паттерн для создания отложенных ордеров
  - Обработка клавиши P в HotkeysService
  - Новый паттерн: P → клик → клик → анализ и отправка в MT4
  - Создание горизонтальных полос скриншотов вокруг точек клика
  - Сохранение скриншотов в папку Trades с уникальными ID
  - Отмена паттерна клавишей Escape
  - События PendingOrderPatternCompleted и PendingOrderPatternCancelled
  - Логирование всех этапов создания паттерна с тегом "PendingOrder"

- **PendingOrderPatternData** - новая модель для паттерна отложенных ордеров
  - Хранение координат двух кликов мыши
  - Пути к скриншотам (FirstScreenshotPath, SecondScreenshotPath)
  - Статус паттерна (InProgress, Completed, Cancelled)
  - Интеграция с существующей системой PendingOrderPatternStatus

- **ResetDB Enhancement** - улучшение функции сброса базы данных
  - Добавлена очистка папки Trades при нажатии кнопки ResetDB
  - Удаление всех файлов скриншотов из папки Trades
  - Логирование процесса очистки

- **Screenshot Optimization** - оптимизация сохранения скриншотов
  - Функция `CropHorizontalStrip()` для вырезания горизонтальных полос
  - Сохранение только области вокруг точки клика (±7 пикселей по Y)
  - Сохранение полной ширины окна, но только нужной высоты
  - Сохранение скриншотов сразу после каждого клика мыши
  - **Исправление**: Добавлено свойство `OrderId` в `PendingOrderPatternData` для обеспечения уникальности имен файлов
  - **Исправление**: Исправлена логика генерации имен файлов - теперь используется один `OrderId` для обоих скриншотов паттерна

- **Escape Key Handling Fix** - исправление обработки клавиши Escape
  - Добавлен флаг `_isServiceEscapeSending` для различения пользовательского и сервисного нажатия Escape
  - Сервисные нажатия Escape (отправляемые программно) игнорируются в обработчике `OnEscapeKeyPressed`
  - Предотвращение ложного срабатывания отмены паттерна при программной отправке Escape
  - Логирование всех операций с тегом "PendingOrder"

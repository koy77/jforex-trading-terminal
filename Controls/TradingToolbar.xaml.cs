...
namespace ScreenCaptureApp.Controls
{
    public partial class TradingToolbar : UserControl
    {
        ...
        // Храним текущие значения для правильного обновления UI торгового паттерна
        private double _currentEntryPrice = 0;
        private double _currentStopLossPrice = 0;
        ...
        /// <summary>
        /// Показывает статус торгового паттерна (BUY/SELL)
        /// </summary>
        /// <param name="tradeType">Тип сделки: "buy" или "sell"</param>
        public void ShowTradingPatternStatus(string tradeType)
        {
            if (Dispatcher.CheckAccess())
            {
                UpdateTradingPatternStatusUI(tradeType, 0, 0);
            }
            else
            {
                Dispatcher.Invoke(() => UpdateTradingPatternStatusUI(tradeType, 0, 0));
            }
        }

        /// <summary>
        /// Обновляет Entry цену в статусе торгового паттерна
        /// </summary>
        /// <param name="entryPrice">Цена входа</param>
        public void UpdateTradingPatternEntry(double entryPrice)
        {
            if (Dispatcher.CheckAccess())
            {
                UpdateTradingPatternStatusUI(null, entryPrice, 0);
            }
            else
            {
                Dispatcher.Invoke(() => UpdateTradingPatternStatusUI(null, entryPrice, 0));
            }
        }

        /// <summary>
        /// Обновляет Stop Loss цену в статусе торгового паттерна
        /// </summary>
        /// <param name="stopLossPrice">Цена стоп-лосса</param>
        public void UpdateTradingPatternStopLoss(double stopLossPrice)
        {
            if (Dispatcher.CheckAccess())
            {
                UpdateTradingPatternStatusUI(null, 0, stopLossPrice);
            }
            else
            {
                Dispatcher.Invoke(() => UpdateTradingPatternStatusUI(null, 0, stopLossPrice));
            }
        }

        /// <summary>
        /// Скрывает статус торгового паттерна
        /// </summary>
        public void HideTradingPatternStatus()
        {
            if (Dispatcher.CheckAccess())
            {
                TradingPatternStatusPanel.Visibility = Visibility.Collapsed;
                // Сбрасываем сохраненные значения
                _currentEntryPrice = 0;
                _currentStopLossPrice = 0;
                TradingPatternEntryLabel.Text = "Entry: ---";
                TradingPatternStopLossLabel.Text = "SL: ---";
                Logger.LogTagInfo("TradingToolbar", "Trading pattern status hidden");
            }
            else
            {
                Dispatcher.Invoke(() =>
        ...
        /// <summary>
        /// Обновляет UI статуса торгового паттерна
        /// </summary>
        private void UpdateTradingPatternStatusUI(string tradeType, double entryPrice, double stopLossPrice)
        {
            // Обновляем тип сделки, если передан
            if (!string.IsNullOrEmpty(tradeType))
            {
                string displayType = tradeType.ToUpper() == "BUY" ? "BUY" : "SELL";
                TradingPatternTypeLabel.Text = displayType;
                TradingPatternTypeLabel.Foreground = displayType == "BUY" ? Brushes.LightGreen : Brushes.LightCoral;
            }

            // Обновляем Entry цену, если передан
            if (entryPrice > 0)
            {
                _currentEntryPrice = entryPrice;
                TradingPatternEntryLabel.Text = $"Entry: {entryPrice:F5}";
            }
            else if (entryPrice == 0 && _currentEntryPrice > 0)
            {
                // Используем сохраненное значение
                TradingPatternEntryLabel.Text = $"Entry: {_currentEntryPrice:F5}";
            }

            // Обновляем Stop Loss цену, если передан
            if (stopLossPrice > 0)
            {
                _currentStopLossPrice = stopLossPrice;
                TradingPatternStopLossLabel.Text = $"SL: {stopLossPrice:F5}";
            }
            else if (stopLossPrice == 0 && _currentStopLossPrice > 0)
            {
                // Используем сохраненное значение
                TradingPatternStopLossLabel.Text = $"SL: {_currentStopLossPrice:F5}";
            }

            // Показываем панель
            TradingPatternStatusPanel.Visibility = Visibility.Visible;
            
            Logger.LogTagInfo("TradingToolbar", $"Trading pattern status updated: Type={tradeType ?? "unchanged"}, Entry={_currentEntryPrice:F5}, SL={_currentStopLossPrice:F5}");
        }
    }
} 
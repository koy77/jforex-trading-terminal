...
### TradingToolbar
- Позиционируется по центру экрана горизонтально (`Canvas.Left="0"` и `Canvas.Right="0"`)
- Располагается в верхней части экрана (`Canvas.Top="0"`)
- Автоматически центрируется на экране, где находится окно JForex

### TradeRectangleControl
- Позиционируется справа по центру экрана (`Canvas.Right="20"`)
- По вертикали центрируется относительно экрана (не окна)
- Окно SimpleTradingOverlay покрывает весь экран
